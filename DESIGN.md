...
0. **Open.** `HorizontalStarTracker` opens the video and reports its resolution, frame rate, and duration back to the UI immediately (shown in the processing window and kept visible for the rest of the run). It then checks that duration against the same period/36 threshold the ring/station table already suggests recording (`RingMath.MinimumReliableVideoDurationSeconds`, falling back to the solver's default 600s seed period if no Kepler/body estimate exists) and fails fast with an actionable message - "record at least Xs and try again" - instead of running the full pipeline on footage too short to show a measurable amount of drift.
